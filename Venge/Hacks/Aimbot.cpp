...
Vector aimhelper::calculateRelativeAngle(const Vector& source, const Vector& destination, const Vector& viewAngles) noexcept
{
...
void aimhelper::run(UserCmd* cmd) noexcept
{
    ...
    if (!config.aimhelper[weaponIndex].enabled)
        weaponIndex = weaponClass;

    if (!config.aimhelper[weaponIndex].enabled)
        weaponIndex = 0;

    if (!config.aimhelper[weaponIndex].betweenShots && activeWeapon->nextPrimaryAttack() > memory.globalVars->serverTime())
        return;

    if (!config.aimhelper[weaponIndex].ignoreFlash && localPlayer->flashDuration())
        return;

    if (config.aimhelper[weaponIndex].onKey) {
        if (!config.aimhelper[weaponIndex].keyMode) {
            if (!GetAsyncKeyState(config.aimhelper[weaponIndex].key))
                return;
        } else {
            static bool toggle = true;
            if (GetAsyncKeyState(config.aimhelper[weaponIndex].key) & 1)
                toggle = !toggle;
            if (!toggle)
                return;
        }
    ...
    if (config.aimhelper[weaponIndex].enabled && (cmd->buttons & UserCmd::IN_ATTACK || config.aimhelper[weaponIndex].autoShot || config.aimhelper[weaponIndex].aimlock) && activeWeapon->getInaccuracy() <= config.aimhelper[weaponIndex].maxAimInaccuracy) {

        if (config.aimhelper[weaponIndex].scopedOnly && activeWeapon->isSniperRifle() && !localPlayer->isScoped())
            return;

        auto bestFov = config.aimhelper[weaponIndex].fov;
        Vector bestTarget{ };
        ...
        aimPunch.x *= config.aimhelper[weaponIndex].recoilControlY;
        aimPunch.y *= config.aimhelper[weaponIndex].recoilControlX;

        for (int i = 1; i <= interfaces.engine->getMaxClients(); i++) {
            auto entity = interfaces.entityList->getEntity(i);
            if (!entity || entity == localPlayer || entity->isDormant() || !entity->isAlive()
                || !entity->isEnemy() && !config.aimhelper[weaponIndex].friendlyFire || entity->gunGameImmunity())
                continue;

            auto boneList = config.aimhelper[weaponIndex].bone == 1 ? std::initializer_list{ 8, 4, 3, 7, 6, 5 } : std::initializer_list{ 8, 7, 6, 5, 4, 3 };

            for (auto bone : boneList) {
                auto bonePosition = entity->getBonePosition(config.aimhelper[weaponIndex].bone > 1 ? 10 - config.aimhelper[weaponIndex].bone : bone);
                if (!entity->isVisible(bonePosition) && (config.aimhelper[weaponIndex].visibleOnly || !canScan(localPlayer, entity, bonePosition, activeWeapon->getWeaponData(), config.aimhelper[weaponIndex].killshot ? entity->health() : config.aimhelper[weaponIndex].minDamage)))
                    continue;

                auto angle = calculateRelativeAngle(localPlayerEyePosition, bonePosition, cmd->viewangles + (config.aimhelper[weaponIndex].recoilbasedFov ? aimPunch : Vector{ }));
                auto fov = std::hypotf(angle.x, angle.y);
                if (fov < bestFov) {
                    bestFov = fov;
                    bestTarget = bonePosition;
                }
                if (config.aimhelper[weaponIndex].bone)
                    break;
            }
        }

        if (bestTarget && (config.aimhelper[weaponIndex].ignoreSmoke
            || !memory.lineGoesThroughSmoke(localPlayer->getEyePosition(), bestTarget, 1))) {
            static Vector lastAngles{ cmd->viewangles };
            static int lastCommand{ };

            if (lastCommand == cmd->commandNumber - 1 && lastAngles && config.aimhelper[weaponIndex].silent)
            ...
            angle /= config.aimhelper[weaponIndex].smooth;
            cmd->viewangles += angle;
            if (!config.aimhelper[weaponIndex].silent)
                interfaces.engine->setViewAngles(cmd->viewangles);

            if (config.aimhelper[weaponIndex].autoScope && activeWeapon->nextPrimaryAttack() <= memory.globalVars->serverTime() && activeWeapon->isSniperRifle() && !localPlayer->isScoped())
                cmd->buttons |= UserCmd::IN_ATTACK2;

            if (config.aimhelper[weaponIndex].autoShot && activeWeapon->nextPrimaryAttack() <= memory.globalVars->serverTime() && !clamped && activeWeapon->getInaccuracy() <= config.aimhelper[weaponIndex].maxShotInaccuracy)
                cmd->buttons |= UserCmd::IN_ATTACK;

            if (clamped)
                cmd->buttons &= ~UserCmd::IN_ATTACK;

            if (clamped || config.aimhelper[weaponIndex].smooth > 1.0f) lastAngles = cmd->viewangles;

...
static void __stdcall drawModelExecute(void* ctx, void* state, const ModelRenderInfo& info, matrix3x4* customBoneToWorld) noexcept
{
    if (interfaces.engine->isInGame()) {
        ...
        static colouredbody colouredbody;
        if (colouredbody.render(ctx, state, info, customBoneToWorld))
...
{
    static auto timereverseInit = (timereverse::init(), false);
...
Hooks::Hooks() noexcept
{
    ...
    **reinterpret_cast<decltype(reset)***>(memory.reset) = reset;
    ...
    //interfaces.gameUI->messageBox("Loaded!", "Venge has loaded.");
}
